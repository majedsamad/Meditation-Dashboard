...
df_monthly_total = (
    df_combined.set_index("Date").resample("MS")[["Hours"]].sum().reset_index()
)
...
if not df_retreat_expanded.empty:
    df_retreats_monthly = (
        df_retreat_expanded.groupby([pd.Grouper(key="Date", freq="MS"), "Kind"])[
            ["Hours"]
        ]
        .sum()
        .reset_index()
...
color_map = {"Sat": "#5D3FD3", "Served": "#FF4B4B"}  # Deep Purple  # Red/Orange

# Apply icon mapping
df_retreats["Icon"] = df_retreats.apply(
    lambda row: get_retreat_icon(row["Name"]), axis=1
)

# 2. Create Single Row Timeline
# We use a timeline for the background bars (duration)
hover_cols = [
    c for c in ["Name", "Link", "Location", "Duration_Days"] if c in df_retreats.columns
]
...
fig_single.update_layout(
    ...
    legend=dict(
        orientation="h", yanchor="bottom", y=1.5, xanchor="left", x=0.0, title=""
    ),
)
...
if not df_retreat_expanded.empty:
    ...
    served_hours = df_retreat_expanded[df_retreat_expanded["Kind"] == "Served"][
        "Hours"
    ].sum()
else:
    sat_hours = 0
...
with col5:
    ...
    years = (
        df_monthly_total["Date"].max() - df_monthly_total["Date"].min()
    ).days / 365.25
...
df_location_agg = (
    df_retreats.groupby(["Lat", "Lon"])
    .apply(build_location_summary, include_groups=False)
    .reset_index()
)
...
fig_map.update_layout(
    ...
    legend=dict(
        orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0, title=""
    ),
)
